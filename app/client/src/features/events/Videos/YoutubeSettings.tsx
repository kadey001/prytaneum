...
import type { YoutubeSettingsFragment$key } from '@local/__generated__/YoutubeSettingsFragment.graphql';
...
export const YOUTUBE_SETTINGS_FRAGMENT = graphql`
    ...
    @argumentDefinitions(first: { type: "Int", defaultValue: 10 }, after: { type: "String", defaultValue: "" }) {
        id
        videos(first: $first, after: $after) @connection(key: "YoutubeSettingsFragment_videos") {
            __id
            edges {
                node {
                    id
...
interface YoutubeSettingsProps {
    fragmentRef: YoutubeSettingsFragment$key;
}

export const YoutubeSettings = ({ fragmentRef }: YoutubeSettingsProps) => {
    const { videos, id: eventId } = useFragment(YOUTUBE_SETTINGS_FRAGMENT, fragmentRef);
        ...
        <Grid container justifyContent='center' alignItems='center' direction='column'>
            <Grid item>
                <Typography variant='body2' color='textSecondary'>
                    Note: Streaming via youtube can have a 10-20 second delay.
                </Typography>
            </Grid>
